...
def inicioAdministrador(request):
    if request.user.is_authenticated:
        try:
            ...
            rol = "Sin grupo asignado"
        
        datosSesion = {"user": request.user, "rol": rol}
...
def registro_usuario(request):
    ...
    tipoUsuario = [('Administrativo',),('Instructor',)]
    roles = [
    ...
    return render(request, 'frmRegistroUsuario.html', context)
...
def vistaGestionarUsuarios(request):
    if request.user.is_authenticated:
        ...
        grupo = request.user.groups.first()  # Obtener el primer grupo del usuario, si existe
        rol = grupo.name if grupo else "Sin grupo asignado"  # Verificar si el grupo existe
...
def vistaRegistrarUsuario(request):
    if request.user.is_authenticated:
        ...
        # Usar .first() en lugar de .get() para evitar excepción si no hay grupos
        grupo = request.user.groups.first()  
        rol = grupo.name if grupo else "Sin grupo asignado"
